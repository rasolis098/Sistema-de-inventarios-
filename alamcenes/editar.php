...
// Verificar si se proporciona el ID del almacén y es válido
if (!isset($_GET['id']) || !is_numeric($_GET['id'])) {
    echo "<div class='alert alert-danger'>ID de almacén no especificado o inválido.</div>";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validaciones
    if (empty($nombre)) {
        $errores[] = "El nombre del almacén es obligatorio.";
    } elseif (strlen($nombre) > 100) {
        $errores[] = "El nombre no puede exceder los 100 caracteres.";
    }
    
    if (strlen($ubicacion) > 255) {
        $errores[] = "La ubicación no puede exceder los 255 caracteres.";
    }
    
    if (strlen($responsable) > 100) {
        $errores[] = "El nombre del responsable no puede exceder los 100 caracteres.";
    }
    
    if (strlen($telefono) > 20) {
        $errores[] = "El teléfono no puede exceder los 20 caracteres.";
    }
    ...
    if (strlen($observaciones) > 500) {
        $errores[] = "Las observaciones no pueden exceder los 500 caracteres.";
    }

    if (empty($errores)) {
        try {
            $sql = "UPDATE almacenes SET nombre = ?, ubicacion = ?, responsable = ?, telefono = ?, 
                    email = ?, capacidad = ?, tipo = ?, estado = ?, observaciones = ? 
                    WHERE id = ?";
            $stmt = $pdo->prepare($sql);
            
            if ($stmt->execute([$nombre, $ubicacion, $responsable, $telefono, $email, 
                               $capacidad, $tipo, $estado, $observaciones, $id])) {
                $mensaje_exito = "Almacén actualizado correctamente.";
            } else {
                $errores[] = "Error al actualizar el almacén. Por favor, intente nuevamente.";
            }
        } catch (PDOException $e) {
            error_log("Error al actualizar almacén: " . $e->getMessage());
            $errores[] = "Error al actualizar el almacén: " . $e->getMessage();
        }
    }
}

// Obtener los datos actuales del almacén
$stmt = $pdo->prepare("SELECT * FROM almacenes WHERE id = ?");
$stmt->execute([$id]);
$almacen = $stmt->fetch();

if (!$almacen) {
    echo "<div class='alert alert-warning'>Almacén no encontrado.</div>";
    include '../includes/footer.php';
    exit;
}
?>
...
<head>
    ...
    <title>Editar Almacén - Sistema de Inventario</title>
    ...
    <div class="container">
        ...
        <div class="page-header">
            <div class="row align-items-center">
                <div class="col-md-8">
                    <h1 class="h3 fw-bold"><i class="fas fa-warehouse me-2 text-primary"></i>Editar Almacén</h1>
                    <p class="mb-0 text-muted">Actualice la información del almacén seleccionado</p>
                </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-4">
                            ...
                            <select name="tipo" class="form-select">
                                ...
                                <option value="principal" <?= $almacen['tipo'] === 'principal' ? 'selected' : '' ?>>Principal</option>
                                <option value="secundario" <?= $almacen['tipo'] === 'secundario' ? 'selected' : '' ?>>Secundario</option>
                                <option value="temporal" <?= $almacen['tipo'] === 'temporal' ? 'selected' : '' ?>>Temporal</option>
                                <option value="refrigerado" <?= $almacen['tipo'] === 'refrigerado' ? 'selected' : '' ?>>Refrigerado</option>
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="col-md-12 mb-4">
                            ...
                            <div class="input-group-icon">
                                ...
                                <textarea name="ubicacion" class="form-control" rows="3" 
                                          ...
                                          maxlength="255"><?= htmlspecialchars($almacen['ubicacion']) ?></textarea>
                            </div>
                            <div class="character-count" id="ubicacion-counter">
                                <?= strlen($almacen['ubicacion']) ?>/255 caracteres
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="col-md-6 mb-4">
                            ...
                            <div class="input-group-icon">
                                ...
                                <input type="text" name="responsable" class="form-control" 
                                       value="<?= htmlspecialchars($almacen['responsable']) ?>" 
                        ...
                        <div class="col-md-6 mb-4">
                            ...
                            <div class="input-group-icon">
                                ...
                                <input type="tel" name="telefono" class="form-control" 
                                       value="<?= htmlspecialchars($almacen['telefono']) ?>" 
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-4">
                            ...
                            <div class="input-group-icon">
                                ...
                                <input type="email" name="email" class="form-control" 
                                       value="<?= htmlspecialchars($almacen['email']) ?>" 
                        ...
                        <div class="col-md-6 mb-4">
                            ...
                            <select name="estado" class="form-select">
                                ...
                                <option value="mantenimiento" <?= $almacen['estado'] === 'mantenimiento' ? 'selected' : '' ?>>En Mantenimiento</option>
                            </select>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-4">
                            ...
                            <div class="input-group-icon">
                                ...
                                <input type="number" name="capacidad" class="form-control" 
                                       value="<?= htmlspecialchars($almacen['capacidad']) ?>" 
                        ...
                        <div class="col-md-6 mb-4">
                            ...
                            <textarea name="observaciones" class="form-control" rows="3" 
                                      ...
                                      maxlength="500"><?= htmlspecialchars($almacen['observaciones']) ?></textarea>
                            <div class="character-count" id="observaciones-counter">
                                <?= strlen($almacen['observaciones']) ?>/500 caracteres
                            </div>
                        </div>
                    </div>
                </div>
